...
const TopProducts = () => {
  ...
  const fetchTopProducts = async () => {
    try {
      // Fetch all reviews
      const response = await axios.get('https://dollar-wala-server.vercel.app/api/reviews');
      ...
      const top3ProductsDetails = await Promise.all(
        Array.from(productMap)
          .map(async ([productId, productData]) => {
            ...
            const productDetailsResponse = await axios.get(`https://dollar-wala-server.vercel.app/api/products/${productId}`);
            return {
